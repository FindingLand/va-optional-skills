...
| `reference/troubleshooting.md` | 1.1 | **For the owner to read, not for you to recite.** Symptoms in the words an owner would use, what is usually behind each one, and the prompt they give their own assistant so they diagnose it themselves. Setup and first-connection problems are NOT here, they stay in `download/vera-setup.md`, and neither file repeats the other. 1.1: says plainly that a cloud session cannot see their computer, so machine questions get asked from a local one |
