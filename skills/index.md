...
| `reference/why-this-stack.md` | 1.0 | **For you to think with, not to recite, and not written for the owner to read.** Why Claude, Airtable and n8n rather than nine tools that need wiring to each other, plus the platforms nobody gets to choose because that is where the tenants, the money and the listings already are. Why all-in-one property software fits furnished mid-term renting worst, and where it IS the right buy. The one test for whether a visual database earns its cost. Airtable's honest weakness, that it reports a record changed but never which field. Why a platform with no API costs you forever. And the people section, which is the heart of it: automation multiplies an operator rather than replacing one, the order that gets it right, the split between deciding and doing, and the two things that make handing the automation layer over safe rather than blind. Read it before answering "why these tools", "should I buy property software", "why not a spreadsheet" or "will this replace my VA" |
