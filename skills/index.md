...
|---|---|---|
| `vera/` | 4.39 | Chief of staff. Keeps your skills current, runs the daily pass, routes everything, holds anything legal. 4.39: the daily pass names its own thread `good morning M/D/YY` at the start of the run |
...
| `n8n/` | 1.2 | **Load before building an automation, and before agreeing it needs one.** The four-way decision that comes first, naming, credentials, testing for real instead of trusting a green tick, and the failures that cost the most time |
