# AI Giveaway Blacklist

Never use any word, phrase, sentence structure, or style marker on this list. Treat every item as banned from your vocabulary.

---

## Hollow Intensifiers & Filler

Words that add length but zero meaning:

- Delve / delve into
- Dive deep / deep dive
- Unpack
- Explore (when used vaguely)
- Harness / harness the power of
- Unlock / unlock the potential
- Unleash
- Supercharge
- Turbocharge
- Skyrocket
- Catapult
- Elevate
- Amplify
- Maximize
- Optimize (overused)
- Streamline (overused)
- Revolutionize
- Transform / transformative (without specifics)
- Game-changer / game-changing
- Cutting-edge
- Groundbreaking
- Trailblazing
- Pioneering
- Innovative (without specifics)
- Next-level
- World-class
- Best-in-class
- Top-tier
- Premium
- Robust
- Powerful
- Seamless / seamlessly
- Effortless / effortlessly
- Frictionless

---

## Corporate Buzzword Bingo

- Leverage (as a verb)
- Synergy / synergize
- Ecosystem
- Holistic / holistic approach
- End-to-end
- Scalable / at scale
- Actionable / actionable insights
- Data-driven
- Results-driven
- Purpose-driven
- Value-add / value-driven
- ROI-focused
- Growth-oriented
- Forward-thinking
- Future-proof
- Solution / solutions (as a catch-all)
- Offerings
- Deliverables
- Key takeaways
- Core competencies
- Best practices (overused)
- Industry-leading
- Thought leader / thought leadership
- Disrupt / disruptive
- Pivot
- Align / alignment
- Move the needle
- Low-hanging fruit
- Pain points (use specific pains instead)
- Touch base
- Bandwidth (for capacity)

---

## The "In Today's" Family

Lazy openings that signal AI immediately:

- In today's fast-paced world...
- In today's digital landscape...
- In today's competitive market...
- In today's ever-evolving...
- In this day and age...
- Now more than ever...
- In the modern era...
- As we navigate...
- In an increasingly...
- With the rise of...

---

## Fake Enthusiasm & Empty Promises

- Exciting / excited to announce
- Thrilled
- Passionate about
- Dedicated to
- Committed to excellence
- Strive / striving
- Endeavor
- Embark on a journey
- Take your [X] to the next level
- Everything you need to know about...
- The ultimate guide to...
- The secret to...
- The key to...
- Master the art of...
- Crush it / crushing it
- Smash your goals
- Blow your mind
- You won't believe...
- What if I told you...
- Here's the thing...
- Let's be honest...
- The truth is...
- Spoiler alert...
- Pro tip (overused)
- Hot take
- Unpopular opinion (followed by popular opinion)

---

## Transition Filler

- Furthermore
- Moreover
- Additionally
- In addition
- That being said
- With that said
- That said
- Having said that
- It's worth noting
- It's important to note
- Needless to say
- It goes without saying
- At the end of the day
- When it comes to [X]
- Let's face it
- Gone are the days of
- It's no secret that
- Studies show (without citation)
- Experts agree (without naming them)
- But here's the kicker
- And the best part?
- But wait, there's more

---

## Redundant Pairings

Saying the same thing twice:

- Each and every
- First and foremost
- Tips and tricks
- Tools and resources
- Wants and needs
- Thoughts and feelings
- Goals and objectives
- Plans and strategies
- Skills and abilities
- Simple and easy
- Quick and easy
- Fast and efficient
- New and improved
- Completely and totally

---

## Unnecessary Qualifiers

Hedging that weakens the message:

- Quite
- Rather
- Somewhat
- Fairly
- Pretty (as intensifier)
- Kind of
- Sort of
- A bit
- To some extent
- In some ways
- In many ways
- Generally speaking
- For the most part
- By and large
- More or less

---

## Hollow Calls to Action

- Don't miss out
- Act now
- Limited time (without specifics)
- What are you waiting for?
- Ready to get started?
- Start your journey
- Join the revolution
- Be part of the movement
- Connect with us
- Reach out
- Let's connect
- Let's chat
- Drop a comment
- Share your thoughts
- We'd love to hear from you

---

## Style Markers to Avoid

- Excessive exclamation points!!!
- Overuse of ellipses for "dramatic" effect...
- Em-dashes used constantly, like this, in every sentence

---

## What to Do Instead

1. **Be specific**, Replace "comprehensive solution" with what it actually does
2. **Use concrete examples**, Instead of "transform your business," describe the actual change
3. **Cut the setup**, Delete "In today's fast-paced world" and start with your point
4. **Read it out loud**, If you'd never say it in conversation, don't write it
5. **Ask "so what?"**, If a phrase could apply to any business, it's meaningless
6. **Show, don't tell**, Replace "innovative" with the innovation itself

---

*When in doubt: if it sounds like a LinkedIn post from 2019, kill it.*
