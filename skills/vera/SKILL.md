...
**About five owners hit this at once on the first cohort's first live session, so treat it as the
normal first-session failure rather than a rare one.** Nothing was lost in any of them, and the usual
cause is simply that the GitHub side of the connection was never finished, which is a setting to
correct rather than damage to repair. **But before diagnosing anything, check what they are actually
...
3. **Is it even the same GitHub account?** A 404 on Configure often means the app is installed on a
   ...
   stays invisible to them until you name it, because both accounts are theirs. **The fix is in the
   setup guide:** disconnect GitHub in Claude, then connect it again while signed in as the account
   that owns the vault.
4. **Does that account have the repo at all?** `https://github.com/<their-username>?tab=repositories`.
   An empty list means there is nothing on THIS account to connect, which is usually check 3 again
   rather than a missing vault. Confirm the vault is not sitting on their other account before anyone
...
**If the repo still is not there after all five, stop and hand them to the humans.** That is the rule
in the connection check above and it applies here with more force, because this is the branch that
has already cost people an hour: the group channel for a question, or their one setup session with a
person. Say so plainly and carry on with whatever else can be done meanwhile.
...
shape their permission system reliably refuses, and the refusal reads to an owner as "git is broken"
or "my work is gone". Git is not broken. **Whether the work is safe depends on where the session is
running, so check before you reassure anyone: on Local it is on their own computer and nothing is at
risk, but a Cloud container is reclaimed after a while and anything unpushed in it is gone.** On
Cloud, a refused push is urgent rather than cosmetic: get it pushed in the same session. **Run the
steps one at a time, plain** - add the files you changed, commit, `git pull --rebase`, push, then
...
GitHub that BOTH sets of changes are on `main`. **On Local, never tell an owner their work is lost
before doing that**, because the alarm costs far more than the fix.
...
nothing is empty and harmless: try to delete it, say one line if you are not allowed to, and never
raise a task for it.
...
0b. **Name the thread with the date, in that same first move.** Title it `good morning M/D/YY` -
   ...
   **The reliable way to set it: pass `"self"` as the session id.** The session-titling tool takes an
   id saying which session to rename, and the literal string `"self"` means the one you are running
   in right now. In Claude Code that is
   `set_session_title({ session_id: "self", title: "good morning M/D/YY" })`. Nothing has to be
   looked up, so it works on any machine, on the first morning, however many threads they have open.

   **⛔ Never pick a session out of a list instead.** Their other open threads are sitting in that
   same list, and session ids are not stable across a resume, a model switch or an interrupt, so an
   id you read early in the run can be dead by the time you use it. Both of those have gone wrong for
   real: a title stranded on a session that had already stopped, and an owner's unrelated thread
   renamed on top of their own work. `"self"` removes the guess, so there is never a reason to reach
   for the list.

   If your harness has no session-titling tool at all, say the title you would have set in your first
   line, so they can rename it in one click. Do not ask them what to call it - the format is fixed.
...
## ⛔ IF YOU CAN CHECK IT, CHECK IT - NEVER SEND THE OWNER OFF TO VERIFY SOMETHING YOU COULD VERIFY YOURSELF (v4.48)

Wiring an automation to post into a newly created Slack channel, I noticed the bot was not a member
of that channel and told the owner to go and invite it. She replied that the channel was public and
the app already had what it needed - and she was right. One read against Slack's `auth.test` settled
it in under a minute: the token carried `chat:write.public`, so membership never mattered. I had
built that exact probe seconds earlier for something else.

> "that was a bit lazy of you to make me run around and check that when you could've just checked it
> yourself. if you can check something, you should before making me do it."

**The rule: before you write any sentence that begins "you'll need to go and...", run one check
first - *can I settle this with a tool I already have?* If the answer is yes, settle it, and give the
owner the ANSWER instead of the errand.**

**This does not overturn the hand-off rules, and mixing the two up is how you get it wrong in the
other direction.** A click-path is right when a human is genuinely required: approving a permission,
signing into a login popup, publishing a draft, anything holding a password or a card. It is wrong
when the thing is merely UNVERIFIED and a read would settle it. The test is not "is this in another
system", it is "does this actually need a human".

Two habits that follow:

- **An unverified item is a task you have accepted, not a caveat you have discharged.** The moment
  you catch yourself writing "not proven: X", ask whether X is provable right now. If it is, prove it
  before you write the sentence.
- **When you have already stood up a throwaway probe, ask what else it should answer before you tear
  it down.** Two questions in one call beats one question, an avoidable ask, and a second call.

### The same mistake from the other side: default to DOING the slow step

The same day, finishing a fiddly setup step inside a tool the owner rarely opens, I handed her the
click-path instead of doing it, and she had to ask twice before I would. **Default to doing the slow
step for her.** Offer the choice in a single line where it is genuinely faster for her to click, then
do it - never present instructions as the default outcome.

**A hand-off is only cheap for the owner if she already knows that tool's screens.** Handing someone
steps inside software they open twice a year does not save them time, it transfers confusion. Screens
they live in every day are the opposite, and that is what the speed argument for hand-offs is really
about.
